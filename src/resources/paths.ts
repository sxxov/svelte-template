...
export default compilePaths();

function compilePaths(): typeof paths {
	...
	const iterateCallback = (key: string, value: Record<string, any> | string): void => {
		...
		const resolvedPath = value.substr(0, 2) === './'
			...
			? pathTool.resolve(`./..${value.substr(1)}`)
	...
	// eslint-disable-next-line
	function iterate(obj: Record<string, any>, callback: (key: string, value: any, index?: number) => void): void {
